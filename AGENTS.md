# Auto Visual Anything Agent Instructions

Auto Visual Anything is a monorepo for related agent skills. The runtime install unit is
...
- Do not make a skill depend on files from repo-root `docs/`, `contracts/`, or
  `scripts/` at runtime.
- Keep API credentials and API probing logic inside `skills/visual-gen`.
- Orchestrators (`visual-anything`, `visual-deck`) may invoke other skills through
  their public skill interfaces, but must not copy generation or credential
  logic.
- Runtime output directories such as `.visual-anything/`, `.visual-deck/`, generated
