---
name: gen-images
description: Use when user requests image generation, image editing, text-to-image (文生图), image editing (改图), 修改图片, 编辑图片, "使用 gpt-image-2", or invokes /gen-images. Calls gpt-image-2 via CLIProxyAPI.
...
调用 CLIProxyAPI 上的 `gpt-image-2` 完成文生图与改图。支持自动触发与 `/gen-images ...` 手动调用。
...
直接从终端调用 `scripts/gen_images.py` 时,脚本不会自己推断;只有通过本 skill 由 LLM 整理参数时才发生这些推断。
...
默认输出目录 `<task_cwd>/gen-images/`。用户指定其他目录时,相对路径按 `task_cwd` 解析。
...
```bash
zsh -lc 'source ~/.zshrc >/dev/null 2>&1 || true; cd "<task-cwd>" || exit 1; python_cmd=$(bash "<skill-dir>/scripts/choose_python.sh") || { echo "未找到可用 Python 3.11+"; exit 1; }; python_argv=(${=python_cmd}); "${python_argv[@]}" "<skill-dir>/scripts/gen_images.py" --mode generate --prompt "..." --out-dir "<task-cwd>/gen-images"'
```
