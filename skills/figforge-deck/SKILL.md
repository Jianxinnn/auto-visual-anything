...
name: figforge-deck
description: >
  figforge-deck: use when the user wants a series of PPT-style slide images (one PNG per
  ...
  "把这篇 paper / repo 做成 deck 图", "工作汇报海报", "商业介绍 cover slide", "/figforge-deck",
  or any request asking for "slide-style images" / "幻灯片图片". Orchestrates figforge-plan
  (source → outline) and figforge-gen (image generation) with a chosen style template.
  Persists run state under <task_cwd>/.figforge-deck/<run-id>/. Do NOT trigger if the user
  wants a real editable .pptx (use the `pptx` skill) or one figure (use figforge / -plan / -gen).
argument-hint: <topic | source path | outline + 可选 style 名>
allowed-tools: [Bash, Read, Write, Edit]
---

# figforge-deck — Source/Topic → Slide-Image Series (orchestrator)

The figforge family extension for **slide-image series**. Each run produces N PNG files,
...
2. Real image generation for each slide (delegated to **figforge-gen**)
3. Optional source-grounded outline drafting (delegated to **figforge-plan**)

If the user wants ONE figure, use `figforge`. If they want an editable `.pptx`, use the
`pptx` skill — `figforge-deck` produces images, not `.pptx`.
...
[STEP 2] Outline   → produce slide list { role, title, captions[] }
  │                   - source material  → delegate to figforge-plan
...
[STEP 3] Render    → for each slide:
  ...
  │                   delegate to figforge-gen → slides/slide-NN.png
  ▼
...
4. **input mode** —
   - source material present (paper / repo / code / diagram / pasted long text) → STEP 2 delegates to `figforge-plan`
...
Source-material detectors are the same as in `figforge` (paths, URLs, ≥ 5 line code blocks,
...
Delegate to `figforge-plan` with the source. Take the resulting Prompt Package's evidence
...
- If the Prompt Package marked `unknown` in critical positions, flag them in the captions as
  ...
  lives in figforge-plan extends to every caption baked into a slide.
...
Orchestrator drafts a 6-slide skeleton inline (no figforge-plan call). The default skeleton:
...
Save the result to `<task_cwd>/.figforge-deck/<run-id>/outline.md`. Stop and ask the user
...
2. Save the compiled prompt to `<task_cwd>/.figforge-deck/<run-id>/prompts/slide-NN.md`.

3. Delegate to **figforge-gen** with these arguments (always go through the `figforge-gen`
   skill — never call `scripts/figforge_gen.py` directly):
   ...
   | `--quality` | `high` |
   | `--output-format` | `png` |
   | `--out-dir` | `<task_cwd>/.figforge-deck/<run-id>/slides/` |
   | `--n` | `1` |

4. After figforge-gen returns its file path, rename / move the file to
   `slides/slide-NN.png` (zero-padded NN). Keep the original figforge-gen run metadata
...
If figforge-gen's `--show-config` preflight fails, surface the error verbatim and STOP.
...
```
<task_cwd>/.figforge-deck/<run-id>/last_run.json
```
...
| Type | Example | Route |
|---|---|---|
| **Cosmetic** | "slide 3 更亮", "重出第 5 张,更冷色" | re-call figforge-gen for that slide ONLY; reuse compiled prompt |
...
Each run lives under `<task_cwd>/.figforge-deck/<run-id>/`:
...
- API credentials live in `figforge-gen` alone. `figforge-deck` never reads, copies,
...
- Do NOT request more than 12 slides per run without explicit user opt-in. (Each slide is
  one figforge-gen call; cost and time scale linearly.)
...
- Do NOT bypass figforge-plan when the user supplied source material. Hand-rolling slide
  content from a paper risks the same hallucination figforge-plan was built to prevent.
...
共生成 N 张幻灯片图像 (style=<style>, size=<size>):
  <task_cwd>/.figforge-deck/<run-id>/slides/slide-01.png
  <task_cwd>/.figforge-deck/<run-id>/slides/slide-02.png
  ...
大纲: <task_cwd>/.figforge-deck/<run-id>/outline.md
```
