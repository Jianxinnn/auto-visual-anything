...
```
<task_cwd>/.figforge-deck/<run-id>/outline.md
```
...
size: 1536x1024                      # one of figforge-gen's supported sizes
slides:
  - role: cover
    title: "Deck Title"
    captions:
...
4. `size` MUST be one of figforge-gen's supported values. The orchestrator passes it
   through; figforge-gen validates. Common choices:
...
The orchestrator delegates outline drafting to `figforge-plan`, which classifies the
...
This preserves the Truthfulness Contract that lives in `figforge-plan` — it extends to
...
The orchestrator drafts the outline directly (no figforge-plan call). Default 6-slide
skeleton:
