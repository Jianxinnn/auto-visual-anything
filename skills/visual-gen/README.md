...
![visual-gen 架构图](assets/architecture.png)
...
- 支持 `visual-gen` 独立 API 配置,优先级高于 Codex / Claude Code
...
把整个 `visual-gen` 目录复制到对应 Agent 的 skills 目录:

| Agent | 路径 |
|-------|------|
| Claude Code(用户级) | `~/.claude/skills/visual-gen/` |
| Codex | 按 Codex 自身 skill 加载规则 |
| Windows Claude Code | `C:\Users\<用户名>\.claude\skills\visual-gen\` |
...
迁移兼容:旧的 `FIGFORGE_GEN_*` 环境变量和 `~/.config/figforge-gen/config.toml`
仍会被读取,但新命名的 `VISUAL_GEN_*` 和 `~/.config/visual-gen/config.toml`
优先级更高。

查看当前实际配置(不展示完整 token):

```bash
python3.12 scripts/visual_gen.py --show-config
```

如果 key 写在 `~/.zshrc`,运行前需在同一 shell source:

```bash
zsh -lc 'source ~/.zshrc >/dev/null 2>&1 || true; python3.12 scripts/visual_gen.py --show-config'
```

## 使用

### 通过 Skill 触发(自然语言或 `/visual-gen`)

```text
使用 gpt-image-2 生成一张透明背景的猫咪头像
/visual-gen 把 ./input.png 改成水彩风,保留主体,输出 webp
```
...
```bash
python3.12 scripts/visual_gen.py \
  --mode generate \
  --model pro/gpt-image-2 \
  --api-base https://your-api-base/v1 \
  --api-key-env VISUAL_GEN_API_KEY \
  --prompt "一张透明背景的猫咪头像" \
  --size 1024x1024 \
  --output-format png \
  --out-dir "./visual-gen"
