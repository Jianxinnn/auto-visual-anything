...
2. **独立环境变量**:`VISUAL_GEN_API_BASE`、`VISUAL_GEN_API_KEY`、`VISUAL_GEN_MODEL`
3. **独立配置文件**:
   - `$VISUAL_GEN_CONFIG`
...
兼容旧配置:脚本仍会在新命名之后回退读取 `FIGFORGE_GEN_*` 环境变量以及
`figforge-gen` 旧配置路径,用于迁移期不中断已有本机配置。新配置优先。
...
如果只设置了 `VISUAL_GEN_API_KEY` 但没有 `VISUAL_GEN_API_BASE` 或配置文件
