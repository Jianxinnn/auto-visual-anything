...
def get_config_paths(args):
    ...
    env_path = os.environ.get("VISUAL_GEN_CONFIG")
    if env_path:
        return [(Path(env_path).expanduser(), True, "VISUAL_GEN_CONFIG")]

    legacy_env_path = os.environ.get("FIGFORGE_GEN_CONFIG")
    if legacy_env_path:
        return [(Path(legacy_env_path).expanduser(), True, "FIGFORGE_GEN_CONFIG")]

    paths = []
    xdg_config_home = os.environ.get("XDG_CONFIG_HOME")
    if xdg_config_home:
        paths.append((Path(xdg_config_home).expanduser() / "visual-gen" / "config.toml", False, "XDG_CONFIG_HOME"))
        paths.append((Path(xdg_config_home).expanduser() / "figforge-gen" / "config.toml", False, "legacy XDG_CONFIG_HOME"))
    paths.extend([
        (Path.home() / ".config" / "visual-gen" / "config.toml", False, "user config"),
        (Path.home() / ".visual-gen" / "config.toml", False, "alternate user config"),
        (Path.home() / ".config" / "figforge-gen" / "config.toml", False, "legacy user config"),
        (Path.home() / ".figforge-gen" / "config.toml", False, "legacy alternate user config"),
    ])
    return paths


def load_visual_gen_config(args):
    for path, required, source in get_config_paths(args):
        if path.exists():
            return flatten_api_config(load_toml(path, f"visual-gen config ({source})")), path
        if required:
            raise RuntimeError(f"未找到 visual-gen 配置文件: {path}")
    return {}, None
...
def resolve_model(args, config: dict):
    return (
        args.model
        or os.environ.get("VISUAL_GEN_MODEL")
...
def load_direct_settings(args, config: dict, config_path: Path | None, model: str):
    base_url = (
        args.api_base
        or os.environ.get("VISUAL_GEN_API_BASE")
    ...
    if args.api_key:
    ...
    elif os.environ.get("VISUAL_GEN_API_KEY"):
        token = os.environ["VISUAL_GEN_API_KEY"]
        token_source = "VISUAL_GEN_API_KEY"
        ...
        if config_key_env:
            token, token_source = read_env_token(config_key_env, "visual-gen config api_key_env")
        else:
            token = get_config_value(config, "api_key", "token")
            if token:
                token_source = "visual-gen config api_key"
    ...
    has_direct_config = any([
        ...
        args.api_key,
        args.api_key_env,
        os.environ.get("VISUAL_GEN_API_BASE"),
    ...
    if not base_url:
        raise RuntimeError("已检测到 visual-gen 独立 API 配置，但缺少 base_url")
    if not token:
        hint = "请设置 api_key、api_key_env，或环境变量 VISUAL_GEN_API_KEY"
        if config_path:
            hint = f"{hint}；当前配置文件: {config_path}"
        raise RuntimeError(f"已检测到 visual-gen 独立 API 配置，但缺少 API key。{hint}")

    source_parts = ["visual-gen"]
    if config_path:
    ...
    if (
        os.environ.get("VISUAL_GEN_API_BASE")
        or os.environ.get("VISUAL_GEN_API_KEY")
        or os.environ.get("FIGFORGE_GEN_API_BASE")
        or os.environ.get("FIGFORGE_GEN_API_KEY")
    ):
...
def load_runtime_settings(args):
    config, config_path = load_visual_gen_config(args)
