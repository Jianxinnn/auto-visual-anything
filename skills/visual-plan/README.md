# visual-plan Skill Project

visual-plan is a source-to-scientific-figure planning skill for agents.
...
The installed skill id is `visual-plan`.

![visual-plan simple style preview](assets/visual-plan-simple-style.png)
...
Default output is a **visual-plan Prompt Package** that the user can paste into an
...
All outputs follow the **visual-plan editorial style**:
...
Copy the `visual-plan/` directory to your agent skills folder. For Codex-style skill
directories, the important entry point is:

```
visual-plan/SKILL.md
```
