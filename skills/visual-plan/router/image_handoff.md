...
**Purpose**: Capture the operational details for handing off a visual-plan prompt
package to an image-generation tool. Read this file only when the user asks
visual-plan to both compile the prompt **and** generate the image.
...
1. Complete visual-plan analysis and compile the full `image_prompt` package first.
...
4. Do not read, copy, transform, or mix API keys between visual-plan and the image tool.
...
## Boundaries with `visual-gen`

When the available image tool is `visual-gen`:

- Run `visual-gen --show-config` as a preflight in the same shell that will perform generation.
- If `VISUAL_GEN_API_KEY` is configured in `~/.zshrc`, source `~/.zshrc` in that same shell.
- Never inject Codex- or Claude-side credentials into `VISUAL_GEN_API_KEY`. The image
